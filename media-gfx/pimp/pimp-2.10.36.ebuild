...
inherit autotools gnome2 toolchain-funcs virtualx unpacker

DESCRIPTION="Perfect Image Manipulation Program"
HOMEPAGE=""
SRC_URI="mirror://gimp/v$(ver_cut 1-2)/gimp-${PV}.tar.bz2"
...
src_prepare() {
	...
	find . -type f | grep "GIMP" | sort | uniq | xargs rename -a GIMP PIMP &> /dev/null
	grep -riIn "gimp" | cut -d ':' -f1 | sort | uniq | xargs sed -i "s/gimp/pimp/g;s/GIMP/PIMP/g;s/Gimp/Pimp/g;s/GiMP/PIMP/g" &>/dev/null
	grep -riIn "GNU Image Manipulation Program" | cut -d ':' -f1 | sort | uniq | xargs sed -i "s/GNU Image Manipulation Program/PIMP - Perfect Image Manipulation Program/g" &>/dev/null
	cp "${FILESDIR}/pimp-splash.png" "data/images"
}
...
_rename_plugins() {
	...
	(
		cd "${ED}"/usr/$(get_libdir)/pimp/2.0/plug-ins || die
...
src_install() {
	...
	# Workaround for bug #321111 to give PIMP the least
	# precedence on PDF documents by default
	mv "${ED}"/usr/share/applications/{,zzz-}pimp.desktop || die

	find "${D}" -name '*.la' -type f -delete || die

	# Prevent dead symlink pimp-console.1 from downstream man page compression (bug #433527)
	local pimp_app_version=$(ver_cut 1-2)
	mv "${ED}"/usr/share/man/man1/pimp-console{-${pimp_app_version},}.1 || die

	# Remove pimp devel-docs html files if user doesn't need it
	if ! use doc; then
